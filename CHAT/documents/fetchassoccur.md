...
`req4c_fetch_assoc_cur()` returns the current ResultRow from `<resultset>` as an associative array.

## Usage

```text
$result = req4c_fetch_assoc_cur(<resultset>);
```
...
`req4c_fetch_assoc_cur()` can be called anytime you have a ResultSet and want to fetch the current ResultRow from `<resultset>` into an associative array.

## Description

`req4c_fetch_assoc_cur()` fetches the current ResultRow from `<resultset>` into an associative array indexed by the column names in the ResultRow. The internal pointer of the current ResultRow is left unchanged.

## Bugs/Features/Comments

`req4c_fetch_assoc_cur()` does not access the network

`req4c_fetch_assoc_cur()` leaves the current message list intact
